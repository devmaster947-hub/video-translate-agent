...
版本 2.3.2 marketplace（本地媒体处理无固定总时限）。产物是完整 Skill 目录的 ZIP，不是 exe，不包含 Python、FFmpeg、OCR 或 ASR 模型。许可证和第三方声明随包保留。依赖范围由 pyproject.toml 指定；不是锁定全部传递依赖的离线安装包。STTN 权重作为同版本 GitHub Release 独立资产发布，GPU 主机按固定 URL 下载并验证 SHA-256。
...
`python scripts/release.py build` 从固定根文件和脚本/包/配置/文档/测试的文本扩展名白名单创建 `dist/staging-v2.3.2-marketplace/video-translate-agent`，随后压缩。存在 staging 或同名 ZIP 时拒绝覆盖；维护者应先检查并将旧发布归档后再重建。
